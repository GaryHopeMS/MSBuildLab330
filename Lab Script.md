...
dotnet build
```
6.	You should see a **Build succeeded** message and **0 Error(s)** when it is complete.
!IMAGE[LAB330BScreenShot0.2.png](instructions261180/LAB330BScreenShot0.2.png)
...
13. Let's ask a general knowledge question, `what is the deepest ocean?`
...
11. Ask this follow up question. `What the second deepest?`, just throwing in a small grammar mistake to make it a little harder. And the response is "the Atlantic Ocean"

!IMAGE[LAB330ScreenShot7.png](instructions261180/LAB330ScreenShot7.png)

12. Create a new chat session, set the data source to **products** and let's ask it something about the products that Cosmic Works sells. `What mountain bikes are available?`
...
1. Open the **Services/SemanticKernel.cs** file.
1. Locate the **SemanticKernelService** constructor method.
1. Locate the section which initializes the Semantic Kernal *memoryStore* and *memory* objects.
1. Review the  memoryStore and memory initialization code that looks like this:
```csharp
...
1. Replace the "// Check if  cache  enabled ..." with the following code:
```csharp
/// Check if  cache  enabled and get a cache hit
if (cacheEnabled)
{
...
1. Replace the ``` if (true) ``` statement  with ``` if (!cacheHit) ``` statement.
...
1. Replace the "// Add entry to cache ..." comments with the following code:
```csharp
//Add entry to cache
...
1. Close any windows of code you have open.

1. In the same terminal from the previous section, start the application using **dotnet run**.
